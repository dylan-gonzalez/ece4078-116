...
class Operate:
    ...
    def drive_to_waypoint(self, waypoint, robot_pose, is_final_waypoint):
        ...
        while True:
            ...
            print("operate.motion_controller([1,0], wheel_ticks, drive_time)")
    ...
    def turn_to_waypoint(self, waypoint, robot_pose):
        ...
        if theta_delta == 0:
            ...
            print("operate.motion_controller([0,1], wheel_ticks, turn_time)")
        elif theta_delta < 0:
            print("operate.motion_controller([0,-1], wheel_ticks, turn_time)")
        else:
    ...
    # Function to convert real-world coordinates to grid coordinates
    def convert_to_grid_space(x, y):
        x_grid = int(np.clip(((x + width / 2) / width) * (n_cells_x - 1), 0, n_cells_x - 1))
        y_grid = int(np.clip(((y + height / 2) / height) * (n_cells_y - 1), 0, n_cells_y - 1))
        return x_grid, y_grid

    # Function to convert grid coordinates to real-world coordinates
    def convert_to_world_space(x, y):
        x_world = x / (n_cells_x - 1) * width - width / 2
        y_world = y / (n_cells_y - 1) * height - height / 2
    ...
    def true_pos(self):
        ...
        return aruco_true_pos, fruits_true_pos
    '''
    def convert_coordinates(x, y, grid_size, scale):
        max_val = scale / 2.0
        x = int((x + max_val) / scale * grid_size)
        y = int((y + max_val) / scale * grid_size)
        return x, y

    def add_obstacle_with_buffer(x, y, buffer_size, grid, grid_size):
...
'''
    ...
    # Add this function to convert real-world coordinates to grid coordinates
    def convert_to_grid_space(self,x, y, width, height, n_cells_x, n_cells_y):
        x_grid = int(((x + width / 2) / width) * (n_cells_x - 1))
        y_grid = int(((y + height / 2) / height) * (n_cells_y - 1))
        return x_grid, y_grid

    # Add this function to convert grid coordinates to real-world coordinates
    def convert_to_world_space(self,x, y, width, height, n_cells_x, n_cells_y):
        x_world = x / (n_cells_x - 1) * width - width / 2
        y_world = y / (n_cells_y - 1) * height - height / 2
        return x_world, y_world

    # Add this function to add obstacles with a buffer to the grid
    def add_obstacles_with_buffer(self,obstacles, buffer_size, grid, width, height, n_cells_x, n_cells_y):
        for obstacle in obstacles:
            x, y = obstacle
            x_grid, y_grid = operate.convert_to_grid_space(x, y, width, height, n_cells_x, n_cells_y)
            ...
            for i in range(-buffer_size, buffer_size + 1):
                for j in range(-buffer_size, buffer_size + 1):
                    ...
                    if 0 <= new_x < n_cells_x and 0 <= new_y < n_cells_y:
                        grid[new_x][new_y] = 1
        return grid
...
if __name__ == "__main__":
    ...
    # Read in the true map
    aruco_true_pos, fruits_true_pos = operate.true_pos()
   ...
   ''' 
    ...
    '''
    # Adding obstacles with a buffer
    buffer_size = 1
    updated_grid = operate.add_obstacles_with_buffer(obstacles, buffer_size, matrix, width, height, n_cells_x, n_cells_y)
    grid = Grid(matrix=updated_grid)
    print(grid)
    
    for row in obstacles:
        print(row)

    start_x, start_y = operate.convert_to_grid_space(x, y, width, height, n_cells_x, n_cells_y)

    #start_x, start_y = operate.convert_to_grid_space(0,0)
    ...
    for idx in range(len(search_list)):
        ...
        x_grid, y_grid = operate.convert_to_grid_space(fruits_true_pos[idx][0]+0.1, fruits_true_pos[idx][1]+0.1, width, height, n_cells_x, n_cells_y)
        ...
        waypoints = []
        for node in path:
            x,y = operate.convert_to_world_space(node.x, node.y, width, height, n_cells_x, n_cells_y)
