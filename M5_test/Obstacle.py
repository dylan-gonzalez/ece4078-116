...
class Polygon:
    ...
    def compute_distance_point_to_polygon(self, point_q, ccw):
        ...
        if ccw:
            for i in range(len_polygon):
                case, seg_dist, _ = self.compute_distance_point_to_segment(self.vertices[(i+1) % len_polygon],self.vertices[i], point_q)
                if seg_dist <= dist:
                    dist = seg_dist
                    if case == 0:
            ...
            for i in range(len_polygon):
                case, seg_dist, _ = self.compute_distance_point_to_segment(self.vertices[i],self.vertices[(i+1) % len_polygon], point_q)
                if seg_dist <= dist:
                    dist = seg_dist
                    if case == 0:
                        closest_point = i
    ...
    def compute_tangent_vector_to_polygon(self, point_q, idx):  
        ...
        case, seg_dist, closest_point = self.compute_distance_point_to_segment(v1, v2, point_q)
    ...
    def compute_inner_vertices(self, offset):
        ...
        self.inner_vertices =  polies[int(self.polygonArea(polies[1][:, 0], polies[1][:, 1], num_points)
                         < self.polygonArea(polies[0][:, 0], polies[1][:, 1], num_points))]
        return self.inner_vertices

    def to_display_format(self, screen_height):
        coordinates = [self.coordinates_to_pygame(v, screen_height) for v in self.vertices[0:-1]]
        return coordinates
    ...
    def is_in_collision_with_points(self, points, min_dist=2.5):
        ...
        for point in points:
            ...
            if count_collisions % 2 == 1:
                self.points_in_collision.append(point)
    ...
    def get_perimeter(self):
        ...
        for i in range(self.vertices.shape[0]-1):
            ...
            perimeter += self.compute_distance_between_points(v1, v2)
...
class Rectangle(Polygon):
    ...
    def to_display_format(self, screen_height):
        py_origin = self.coordinates_to_pygame(self.origin, screen_height)
