...
def drive_to_waypoint(waypoint, robot_pose):
    ...
    distance_to_waypoint = np.sqrt((waypoint_x - robot_pose_x)**2 + (waypoint_y - robot_pose_y)**2)
...
def turn_to_waypoint(waypoint, robot_pose):
    ...
    turn_time = 0
    theta_target = np.arctan2(waypoint_y - robot_pose_y, waypoint_x - robot_pose_x) # angle from robot's current position to the target waypoint
...
def drive_to_waypoint(wp):
    ...
    drive_time = 0
    distance_to_waypoint = math.sqrt((wp[0])**2 + (wp[1])**2)
