...
class Operate:
    ...
    def motion_controller(self, motion, wheel_ticks, drive_time):
        ...
        while time.time()<=drive_time:
            ...
            # could add an if statement to check if it is close to our desired waypoint
            robot_pos = self.get_robot_pose()[0:2]
            dx = float(self.wp[0])-robot_pos[0]
            dy = float(self.wp[1])-robot_pos[1]
   ...
   # operate.motion_controller([0,0], 0.0, 0.0) #TODO: Might not even need this if i rotate
    ...
    for idx in range(5): #TODO dont hardcode the length of the search list
        ...
        for wp in operate.path:
            ...
            new_angle_pose = operate.turn_to_waypoint()
            #operate.robot_pose = operate.get_robot_pose()   
            operate.robot_pose[2] = new_angle_pose
       
            new_pose = operate.drive_to_waypoint()
            operate.robot_pose[0:2] = new_pose
        ...
        # I want it to do a turn so that it reads slam
        cov_x = operate.ekf.P[0,0]
        cov_y = operate.ekf.P[1,1]
        
        if cov_x>1 or cov_y>1: 
            operate.motion_controller([0,1],0,12)
