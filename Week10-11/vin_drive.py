...
class Operate:
    def __init__(self, args):
        ...
        self.angle_kp = 0.5  # Proportional coefficient for angle
        self.angle_ki = 0.01 # Integral coefficient for angle
        self.angle_kd = 0.01 # Derivative coefficient for angle
    ...
    def drive_to_waypoint(self):
        ...
        if dist_error < 0.1:
            print("Waypoint too close, skipping...")
        else:
    ...
    def turn_to_waypoint(self):
        self.target_angle = math.atan2(self.wp[1] - self.robot_pose[1], self.wp[0] - self.robot_pose[0])
        self.theta_delta = self.target_angle - self.robot_pose[2]

        # Normalize angle error to [-pi, pi]
        self.theta_delta = math.atan2(math.sin(self.theta_delta), math.cos(self.theta_delta))
        
        # PID Control for turning
        self.angle_integral += self.theta_delta
        angle_derivative = self.theta_delta - self.prev_angle_error
        turn_rate = self.angle_kp*self.theta_delta + self.angle_ki*self.angle_integral + self.angle_kd*angle_derivative
        self.prev_angle_error = self.theta_delta

        # Turning logic based on PID control
        self.turn_time = abs(turn_rate)
        print("Turning for {:.2f} seconds".format(self.turn_time))
        
        if self.theta_delta == 0:
            print("No turn required")
        elif turn_rate > 0:
            self.motion_controller([0, 1], self.turn_ticks, self.turn_time)
        elif turn_rate < 0:
            self.motion_controller([0, -1], self.turn_ticks, self.turn_time)
        else:
            print("There is an issue with the turning function")
            
        print("Reached angle pose: [{}], Target angle pose is: [{}]".format(self.get_robot_pose()[2], self.target_angle))
...
if __name__ == "__main__":
    ...
    # Robot navigation
    for idx in range(len(operate.search_list)): #TODO dont hardcode the length of the search list
        ...
        smoothed_points = operate.ensure_max_distance(reduced_points, max_distance=0.2)
        ...
        for wp in operate.path:
            ...
            while True:
                ...
                angle_error = float(math.atan2(math.sin(operate.wp[1] - operate.robot_pose[1]),
                                            math.cos(operate.wp[0] - operate.robot_pose[0])) - operate.robot_pose[2])
                
                if abs(angle_error) < math.radians(4):  # Changed the angle error to radians for consistency
                    break
